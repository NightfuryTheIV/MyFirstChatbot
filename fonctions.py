...
"""
def monologue():
    monolog = ["First step was to split your question into tokens that we use in three other functions to process everything we need to in order to give a valid answer.", 
    ...
    for line in monolog:
        ...
        for emptyspaces in range(max_line_length - len(line)):
            emptyspace += " "
        print("| " + line + emptyspace + " |")
    
    print(first_line)


def boot_up():
    action1 = input("Hello there! You can test out our application by typing Y, F to see our features, or any other key to exit. ")
    if action1 == "Y":
